...
class StressTestRunner {
  ...
  async stress(name, testFn, options = {}) {
    const { iterations = 1000, concurrency = 10, timeout = 30000 } = options;
  ...
  report() {
    ...
    this.results.forEach((result) => {
      ...
      console.log(
        `   Errors: ${result.errors}/${result.iterations} (${((result.errors / result.iterations) * 100).toFixed(2)}%)`
      );
    });
