...
const { activityPlugin, logActivity, activityConfig, Activity } = require('../../dist/index.js');
...
class PerformanceMonitor {
  ...
  report() {
    ...
    this.measurements.forEach(m => {
      ...
      console.log(`   Memory delta: RSS ${(m.memoryDelta.rss / 1024 / 1024).toFixed(2)}MB, Heap ${(m.memoryDelta.heapUsed / 1024 / 1024).toFixed(2)}MB`);
    });

    const memUsage = this.getMemoryUsage();
    console.log(`\n💾 Total Memory Usage:`);
    console.log(`   Current RSS: ${(memUsage.current.rss / 1024 / 1024).toFixed(2)}MB`);
    console.log(`   Current Heap: ${(memUsage.current.heapUsed / 1024 / 1024).toFixed(2)}MB`);
    console.log(`   Total RSS Delta: ${(memUsage.delta.rss / 1024 / 1024).toFixed(2)}MB`);
    console.log('=' .repeat(80));
  }
}
...
async function runPerformanceTests() {
  ...
  try {
    ...
    await monitor.measure('Schema Plugin Setup (1000 schemas)', async () => {
    ...
    await monitor.measure('Sync: Create 100 users', async () => {
      const users = [];
      for (let i = 0; i < 100; i++) {
    ...
    await monitor.measure('Async: Create 100 users', async () => {
      const users = [];
    ...
    await monitor.measure('Bulk: Insert 500 products', async () => {
      return await Product.insertMany(products);
    }, 500);

    await monitor.measure('Bulk: Update 500 products', async () => {
    ...
    await monitor.measure('Bulk: Delete 250 products (electronics)', async () => {
    ...
    await monitor.measure('Manual: Log 1000 activities', async () => {
      const promises = [];
      for (let i = 0; i < 1000; i++) {
    ...
    await monitor.measure('Concurrent: 10 parallel user batches (50 each)', async () => {
      const batches = [];
    ...
    await monitor.measure('Heavy: Update 100 users (multiple fields)', async () => {
    ...
    await monitor.measure('Query: Get activity feed (1000 activities)', async () => {
    ...
    await monitor.measure('Memory: Create 2000 activities rapidly', async () => {
      const promises = [];
